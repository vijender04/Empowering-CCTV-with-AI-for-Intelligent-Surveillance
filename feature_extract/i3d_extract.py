...
def load_frame(frame_file):
    ...
    data = data.resize((340, 256), Image.LANCZOS)#Image.ANTIALIAS
...
def forward_batch(b_data,net):
    ...
    with torch.no_grad():
        net.eval()
        b_data = Variable(b_data.to("cuda" if torch.cuda.is_available() else "cpu")).float()#b_data = Variable(b_data.cuda()).float()
...
def run(args_item):
    ...
    # setup the model  
        ...
        i3d.to("cuda" if torch.cuda.is_available() else "cpu")#i3d.cuda() 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', default="feature_extract/UCF_Crime_Frames",type=str)
    ...
    for videos in os.listdir(args.input_dir):
        for video in os.listdir(os.path.join(args.input_dir,videos)):
            ...
            if save_file in os.listdir(os.path.join(args.output_dir)):
                ...
                vid_list.append(args.input_dir+"/"+videos+"/"+video)
    ...
    #for i,n in enumerate(vid_list):
    #    run([args.load_model,n,args.output_dir,args.batch_size,i])